...
files <- list.files(path="~/Documents/WORK/Arabis_resequencing/V5.1/Genotyping/New/zygosity/output_latest_2to1_2to1_twotailed/", pattern="absence.txt")
datd1=read.table("~/Documents/WORK/Arabis_resequencing/V5.1/Genotyping/New/zygosity/reformat_genotyped_del_with_second_pass_flipped1.bed",header=T,sep='\t',stringsAsFactors = F)

datd1$V1=gsub("A", "", as.character(datd1$V1))
datd1$V1=as.numeric(datd1$V1)
DF=as.data.frame(datd1[,c(1)],stringsAsFactors=F)
names(DF)="V1"
...
setwd("~/Documents/WORK/Arabis_resequencing/V5.1/Genotyping/New/zygosity/recalculate4_two_tailed/")
#dti2=read.table("reformat_genotyped_del_with_second_pass_flipped_w_header.bed",header=T,sep='\t')
#heads=names(dti2)
#dput(heads)
dtd=datd1[,c(1,16:319)]
names(dtd)=c("V1","20150213.B-Ma28", "20150213.B-Ma29", "20150213.B-Ma30", "20150213.B-Ma31", 
...
#DF2=DF
#DF2[2:ncol(DF2)] <- 1-DF2[2:ncol(DF2)]
#DF=DF2
#From flipped deletion file make not flipped file as zygosity information are not flipped
noflip.dti=dtd
noflip.dti[2:ncol(noflip.dti)] <- 1-noflip.dti[2:ncol(noflip.dti)]
#t[t==0]=4
#t[t==1]=0
#t[t==4]=1
#noflip.dti[,c(16:319)]=t
mdf=mapply("*", DF[intersect(names(DF), names(noflip.dti))],noflip.dti[intersect(names(noflip.dti), names(DF))])
#mdf=mapply("*", DF[intersect(names(DF), names(dti))],dti[intersect(names(dti), names(DF))])

mdat=as.data.frame(mdf)
mdat$V1=sqrt(mdat$V1)
mdat.t=mdat
mdat.t[2:ncol(mdat.t)] <- 1-mdat.t[2:ncol(mdat.t)]
mdat=mdat.t
mdat2=mdat*2
mdat2$V1=mdat2$V1/2
...
mdat2$V1=paste("A",mdat2$V1,sep='')
write.table(mdat2,"Genotyped_del_with_zyg.txt",row.names = F,sep='\t',quote = F)

mdat3=mdat2[mdat2$MAF>=0.025,]
mdat4=mdat3[mdat3$Missing<=0.1,]
write.table(mdat4,"Genotyped_del_with_zyg_filtered_MAF0.025_Missing0.1.txt",row.names = F,sep='\t',quote = F)
